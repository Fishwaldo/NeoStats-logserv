...
static int ls_cmd_stats( const CmdParams *cmdparams );
...
static bot_cmd ls_commands[]=
{
	...
	{"STATS",	ls_cmd_stats,	0, 	NS_ULEVEL_OPER,	ls_help_stats, 0, NULL, NULL},
	NS_CMD_END()
};
...
int ModSynch( void )
{
	...
	(void)AddTimer( TIMER_TYPE_INTERVAL, ls_RotateLogs, "ls_RotateLogs", 300, NULL );
	return NS_SUCCESS;
}
...
static int ls_cmd_stats( const CmdParams *cmdparams )
{
	irc_prefmsg( ls_bot, cmdparams->source, "Stats:" );
